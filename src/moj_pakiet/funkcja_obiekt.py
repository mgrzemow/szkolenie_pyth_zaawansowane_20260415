"""
Demonstracja tego, ze funkcje w Pythonie sa pelnoprawnymi obiektami.

Ten plik zbiera kilka mniejszych eksperymentow pokazujacych, ze funkcje
mozna traktowac jak dane: przechowywac w zmiennych, przekazywac jako
argumenty, wywolywac posrednio, laczyc z narzedziami biblioteki
standardowej i zamykac w obiektach lub domknieciach.

W module pojawiaja sie miedzy innymi:

- obiekt funkcyjny i atrybut `__call__`,
- wlasny obiekt wywolywalny implementujacy `__call__`,
- funkcje anonimowe `lambda`,
- `defaultdict` z funkcja fabrykujaca wartosc domyslna,
- przekazywanie funkcji do `sort`, `map`, `filter` i `reduce`,
- czesciowe zastosowanie argumentow przez `functools.partial`,
- memoizacja przez `functools.lru_cache`.

To nie jest jeden algorytm, tylko zestaw malych scenek dydaktycznych.
Kazda z nich ma pokazac inna ceche skladni i modelu jezyka:

- funkcja moze byc wartoscia,
- obiekt niebedacy funkcja tez moze byc wywolywalny,
- funkcja moze sluzyc jako "strategia" przekazana do innego API,
- domkniecia i dekoratory z biblioteki standardowej moga modyfikowac
  zachowanie funkcji bez zmiany jej skladni uzycia.

Najwazniejsze elementy skladni to `def`, `lambda`, `*args`, `**kwargs`,
`functools.reduce`, `@functools.lru_cache`, `functools.partial`,
przekazywanie funkcji do argumentu `key=` oraz zwykle wywolywanie
obiektow przez nawiasy.

Modul dobrze nadaje sie do rozmowy o tym, dlaczego Python bywa okreslany
jako jezyk, w ktorym funkcje sa "first-class citizens".
"""
